...
function SiteConfigurationPageContent() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const siteData = await getSiteData({ publisher });
      if (publisher && publisher) {
        setSiteName(publisher);
        if (siteData["videoconfig"]) {
          setVideoUrls({ ...siteData["videoconfig"] });
        }
        if (siteData["metadata"]) {
          setLogoUrl(siteData["metadata"]?.openGraph?.images?.[0]);
        }
        if (siteData["themeconfig"]) {
          setColors({ ...siteData["themeconfig"] });
        }
      }
      setLoading(false);
    })();
