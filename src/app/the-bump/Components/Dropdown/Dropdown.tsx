...
const Dropdown = ({
  ...
  const handleRedirect = (url, isInternal = false) => {
    if (isInternal) {
  ...
  const renderDropdownButton = (label, dropdown) => (
    <button
      onClick={() => toggleDropdown(dropdown)}
      className={`${
        openDropdown === dropdown ? "rounded-b-none" : ""
  ...
  const renderDropdownMenu = (dropdown, items, width) =>
    openDropdown === dropdown && (
      <div
        className={`absolute right-0 mt-0 w-[12rem] bg-white border shadow-lg rounded-md ${
          openDropdown === dropdown ? "rounded-tr-none" : ""
        }`}
        style={{ width }}
      >
          ...
          {items.map((item, index) => (
            <li key={index}>
              <button
                ...
                onClick={() => handleRedirect(item.url, item.isInternal)}
              >
                {item.label}
              </button>
            </li>
  ...
  return (
    <div className={`relative ${offeringsButtonWrap}`}>
      <div className="flex space-x-4">
        ...
        <div className="relative">
          {renderDropdownButton("For Publishers", "publishers")}
          {renderDropdownMenu("publishers", publishersItems, "9.7rem")}
        </div>
      </div>
    </div>
  );
