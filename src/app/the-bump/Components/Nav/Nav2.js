...
const ScrollableNavbar = ({ customBg }) => {
	...
	return (
		<nav
			className={`fixed top-0 w-full lg:shadow-lg text-white py-2 px-3 transition-opacity duration-300 p-2 md:px-8  bg-secondary/50 rounded-b-2xl lg:py-1 lg:flex justify-center z-[100] ${
		...
		>
			<div className="text-sm text-center md:gap-x-6 gap-y-2 items-center pt-0  w-full md:w-fit justify-between lg:justify-center hidden sm:flex sm:flex-row md:flex-row lg:flex">
				...
				{pathname !== '/' && <div className="bg-[#004A6C] h-7 w-0.5 lg:hidden"></div>}
				{pathname !== '/' && (
					<button
						onClick={openModal}
				...
				<div className="bg-[#004A6C] h-7 w-0.5 lg:hidden"></div>
				<Link href={'/features'} className="rounded-[24px]  px-4 py-1 md:text-primaryTextColor font-medium">
