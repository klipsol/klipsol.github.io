...
export const BrandList = () => {
	...
	return (
		<div className="text-[#1D234E] py-4  mt-8 sm:bg-[#EEF0F8] sm:py-10 lg:bg-[#FFF9E6] lg:py-12">
			<h1 className="custom-text-sm-medium font-normal text-center mb-7 sec-container">
				...
				Publishers with <span className="font-semibold">200M+ Users </span> & <br className="lg:hidden" />
					...
					{allBrands.map((brand, index) => (
						<div key={index} className="flex items-center justify-center">
							<div className="bg-white rounded-xl p-3 w-full">
								<img src={brand.img} alt={brand.name} className="h-6 w-full object-contain" />
							</div>
						</div>
					))}
				</div>
							...
							{allBrands.map((brand, index) => (
								<div key={index} className="flex items-center justify-center mx-1">
									<div className="sm:bg-[#FFFDF5] md:bg-transparent rounded-3xl px-5 py-4">
										<img src={brand.img} alt={brand.name} className="h-8 2xl:h-11 object-contain" />
									</div>
								</div>
							))}
						</div>
